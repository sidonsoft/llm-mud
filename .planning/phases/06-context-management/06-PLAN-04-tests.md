...
must_haves:
  truths:
    - "Tests verify relevance filtering works correctly"
    - "Tests verify memory transfer between short and long term"
    - "Tests verify compaction rate limiting"
    - "Tests verify token budget enforcement"
  artifacts:
    - path: "test_context_manager.py"
      provides: "Unit tests for ContextManager"
      min_lines: 100
  key_links:
    - from: "test_context_manager.py"
      to: "context_manager.py"
      via: "imports ContextManager, ActivityType"
      pattern: "from context_manager import"
---

<objective>
Create comprehensive unit tests for the context management system.

Purpose: Verify all context management features work correctly: relevance filtering, memory split, compaction, budget enforcement.
Output: test_context_manager.py with comprehensive tests
</objective>
...
<context>
From test_llm_agent.py (existing test patterns):
```python
import unittest
...
class ContextManager:
    ...
    def add_message(self, content, activity_type, timestamp)
    def _calculate_relevance(self, entry) -> float
    def get_filtered_context(self, current_output) -> List[MemoryEntry]
    def should_compact(self, current_token_estimate, token_budget) -> bool
    def can_compact(self) -> bool
    def trigger_compaction(self, llm_provider) -> str
```
</context>
...
<task type="auto">
  <name>Task 1: Create test_context_manager.py</name>
  <files>test_context_manager.py</files>
  <action>
Create `test_context_manager.py`:
...
class TestCompaction(unittest.TestCase):
    ...
    def test_trigger_compaction_creates_summary(self):
        ...
        mock_provider.chat = AsyncMock(return_value="Summary: Multiple events occurred.")
...
- Relevance scoring (keywords, activity, ambient, goals)
- Memory split (short-term vs long-term transfer)
...
- Goal management
  ...
  <verify>
    <automated>python -m pytest test_context_manager.py -v --tb=short 2>&1 | head -60</automated>
  </verify>
  <done>Comprehensive unit tests for context management created</done>
</task>

</tasks>

<verification>
- All unit tests pass for relevance filtering
- Memory transfer tests pass
- Compaction rate limiting tests pass
- Budget enforcement tests pass
</verification>

<success_criteria>
All context management tests pass, code coverage for new features
</success_criteria>

<output>
After completion, create `.planning/phases/06-context-management/06-04-SUMMARY.md`
</output>
